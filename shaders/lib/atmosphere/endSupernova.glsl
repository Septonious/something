...
void drawEndSupernova(inout vec3 color, in vec3 worldPos, in float VoU, in float VoS, inout float nebulaFactor) {
    ...
    vec3 supernova = star * endLightColSqrt * pulse * pulse * 3.0;
    ...
    vec3 nebula = (1.0 + noiseDiff) * mix(vec3(2.4, 1.5, 0.1), vec3(0.1, 2.8, 1.1), nebulaColorMixer) * baseNoise * offsetNoise * nebulaVisibility;
